(function() {
  ...
  // Configuration endpoint - you can change this to your dashboard's API
  const CONFIG_URL = 'https://your-dashboard-domain.com/api/config.json';
  ...
  window.RedInternational = {
    ...
    loadConfig: function() {
      ...
      if (localConfig) {
        try {
          ...
          console.log('[Red International] Config loaded from localStorage', this.config);
          this.applyConfig();
          return;
        } catch (e) {
      ...
      fetch(CONFIG_URL)
        ...
        .then(config => {
          ...
          console.log('[Red International] Config loaded from remote', this.config);
          this.applyConfig();
        })
        .catch(error => {
          console.warn('[Red International] Failed to load remote config, using defaults', error);
          this.applyConfig();
        });
    },
    ...
    applyConfig: function() {
      Object.keys(this.config).forEach(serviceId => {
        ...
        const elements = document.querySelectorAll(`[data-red-service="${serviceId}"]`);
